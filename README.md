...
The AusTIMES VEDA Table CLI (`austimes-tables`) enables version control workflows for VEDA-TIMES Excel input decks by creating canonical CSV "shadow tables" alongside your Excel files. This provides:
...
├── BY_Trans.xlsx
├── SupplXLS/*.xlsx
└── shadow/                 # Generated by austimes-tables
    ├── tables/
    │   ├── VT_Example/
...
```bash
austimes-tables extract /path/to/deck
austimes-tables validate /path/to/deck
```

### Using pip

```bash
pip install austimes-tables
```
...
**Quick demo**:
```bash
austimes-tables extract examples/sample_deck
austimes-tables validate examples/sample_deck
```
...
austimes-tables diff baseline_2024/ scenario_2025/ --output changes.json
austimes-tables report baseline_2024/ scenario_2025/ --output changes.html
```
...
```bash
austimes-tables extract <deck_root> [--output-dir DIR] [-v]
```
...
austimes-tables extract ~/models/austimes_baseline

# Extract with custom output directory
austimes-tables extract . --output-dir=export

# Extract with verbose output (shows per-table progress)
austimes-tables extract . -v
```
...
```bash
austimes-tables format <deck_root>
```
...
austimes-tables format . && git add shadow/ && git commit -m "Update tables"
```
...
```bash
austimes-tables validate <deck_root>
```
...
austimes-tables validate ~/models/austimes_baseline
...
```bash
austimes-tables diff <deck_a> <deck_b> [--output FILE]
```
...
austimes-tables diff baseline_2024/ scenario_2025/
...
```bash
austimes-tables report <deck_a> <deck_b> --output FILE [--limit-rows N]
```
...
austimes-tables report baseline_2024/ scenario_2025/ --output diff.html

# Limit detailed row output for large diffs
austimes-tables report baseline_2024/ scenario_2025/ --output diff.html --limit-rows 5000
```
...
steps:
  - name: Extract shadow tables
    run: austimes-tables extract deck/
  
  - name: Format for canonical output
    run: austimes-tables format deck/
  
  - name: Validate schema
    run: austimes-tables validate deck/
